...
double find_median(int *freqs, int num_genes) {
  ...
  qsort(freqs, num_genes, sizeof(int), compare_int);

  // TODO: median if even
  if (num_genes % 2 == 0) {
    int f1 = freqs[num_genes / 2 - 1];
    int f2 = freqs[num_genes / 2];
    return (double)(f1 + f2) / 2.0;
  }
...
#pragma omp parallel for default(none) private(gene_index)                     \
    shared(gene_TF_counts, genes)
...
#pragma omp parallel for default(none) shared(gene_TF_counts, median_TF, genes)
